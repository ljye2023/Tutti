 ...
 *   [ 6] open_gpu_file(CREATE) (4 shards over devices[0..3])
...
void usage(const char* prog) {
    std::fprintf(stderr,
        "Usage:\n"
        "  IN_PROCESS: %s [--gpu N] [--cuda N] [--cap N] [--queues N]\n"
        "              [--total-gib F] [--compute-iters N]\n"
        "              <PCI0> <PCI1> <PCI2> <PCI3>\n"
        "  SERVICE_CLIENT: %s [--cuda N] [--queues N] [--total-gib F]\n"
        "              [--compute-iters N] --service <endpoint>\n"
        "              --dev-id N0 --dev-id N1 --dev-id N2 --dev-id N3\n"
        "Defaults: queues=16 total-gib=8 compute-iters=5000000\n"
        "Needs exactly 4 devices (num_shards = 4).\n"
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ) {
        ...
        } else if (std::strcmp(a, "--total-gib") == 0 && i + 1 < argc) {
            total_gib = std::atof(argv[i + 1]); i += 2;
        } else if (std::strcmp(a, "--compute-iters") == 0 && i + 1 < argc) {
            compute_iters = (uint32_t)std::atoi(argv[i + 1]); i += 2;
    ...
    if (n_dev_req != kNumShards) {
        std::fprintf(stderr,
            "need exactly %u devices (got %zu)\n",
            kNumShards, n_dev_req);
    ...
    // [2] registry up -- IN_PROCESS or SERVICE_CLIENT, mode-agnostic
    //     past this point.
    ...
    // [6]
    nvtxRangePushA("alloc:open_gpu_file(CREATE)+cudaMalloc+nvm_dma_map");
    tutti::GpuFileSpec spec{};
    ...
    STEP_OK("open_gpu_file(CREATE) '%s' total=%.2f GiB shards=%u tensor=%u "
    ...
    // [9] configure the async handle pool + acquire_device_handle.
    //     Stream = default (nullptr/0); this smoke launches its actual
    //     IO kernels on dedicated streams (s_io/s_cmp, created later)
    //     that have no inherent ordering vs. stream 0, so synchronize
    //     once right after the async acquire to guarantee the H2D
    //     handle fill has landed before anything reads through it.
    ...
    nvtxRangePop();   // alloc:open_gpu_file(CREATE)+cudaMalloc+nvm_dma_map
    ...
    // false (it deliberately handles only single-extent IOs --
    ...
    for (uint32_t s = 0; s < kNumShards; ++s) {
        ...
        for (std::size_t i = 0; i < exts.size(); ++i) {
            ...
            // No in-band header prefix -- user data starts at
            // byte 0, so every extent byte is payload.
            cum += bytes;
    ...
    // [16] release.  R11: release_device_handle is async -- queue it
    //     on `s_io` (the stream every read/write kernel that touched
    //     `h` ran on) BEFORE destroying that stream, then synchronize
    //     so the pending-return callback has actually run.
    nvtxRangePushA("teardown");
    if (d_compute) cudaFree(d_compute);
    if (d_ctx)     cudaFree(d_ctx);
    bs->release_device_handle(h, s_io); h = nullptr;
    CUDA_OK(cudaStreamSynchronize(s_io));
