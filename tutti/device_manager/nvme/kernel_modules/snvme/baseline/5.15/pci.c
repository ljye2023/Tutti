...
/* struct nvme_dev moved to snvm_ndev.h (shared-layer visibility) */
...
static int nvme_remap_bar(struct nvme_dev *dev, unsigned long size)
{
	...
	if (size > pci_resource_len(pdev, 0))
		return -ENOMEM;
	pr_info("nvme_remap_bar bar size is %llx\n", pci_resource_len(pdev, 0));
	if (dev->bar)
		iounmap(dev->bar);
...
static int s_nvme_setup_io_queues(struct nvme_dev *dev)
{
	...
	if (dev->cap_kernel_ioq &&
		...
		pr_info("capping kernel-side IOQ count from %u to %u "
 ...
 retry:
	...
	pr_info("nvme_setup_io_queues max_id is %d\n",dev->max_qid);
	pr_info("nvme_setup_io_queues online_queues is %d\n",dev->online_queues);
	/*
	...
	pr_info("create io queue finish max_id is %d, online_queues is %d\n",dev->max_qid,dev->online_queues);
...
static int nvme_delete_queue(struct nvme_queue *nvmeq, u8 opcode)
{
	...
	req = snvme_alloc_request(q, &cmd, BLK_MQ_REQ_NOWAIT);
	if (IS_ERR(req))
		return PTR_ERR(req);
...
static void nvme_reset_work(struct work_struct *work)
{
	...
	result = snvme_init_ctrl_finish(&dev->ctrl);
	if (result)
		goto out;
...
static int __init nvme_init(void)
{
	...
	ret = snvm_global_init();
	if (ret)
		return ret;
	/* hand our struct pci_driver (which names the static
	 * probe/remove/shutdown callbacks above) to the shared
	 * control plane; registration stays lazy (first BIND). */
	return snvm_pci_register(&snvme_driver);
}
