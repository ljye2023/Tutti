...
// Forward-declare launch_fill_pattern (DMA-visible GPU buffer fill)
namespace tutti::data_paths::local_nvme {
void launch_fill_pattern(void* buf, unsigned char val, std::uint64_t n, void* stream);
}

// SGEMM kernel (same as legacy)
__global__ void sgemm_kernel(const float* A, const float* B, float* C, int n, int iters) {
    ...
    for(int idx=blockIdx.x*blockDim.x+threadIdx.x; idx<n*n; idx+=s) {
        int r=idx/n, c=idx%n;
        for(int it=0;it<iters;++it){float a=0;for(int k=0;k<n;++k)a+=A[r*n+k]*B[k*n+c];C[r*n+c]=a;}
    }
}
...
int main(int argc,char**argv){
    ...
    // so that a full per-layer per-direction batch (up to 2*n_hit requests,
    ...
    // ---- Phase E: pre-write HIT chunks ----
    {
        ...
        for(uint32_t L=0;L<n_layers;++L){
            ...
            for(uint64_t b=0;b<n_hit;++b){
                ...
                launch_fill_pattern(hk[b],sk,ts,s_w);
                launch_fill_pattern(hv[b],sv,ts,s_w);
            }
    ...
    sgemm_kernel<<<gg,gb,0,s_c>>>(dA,dB,dC,(int)gemm_n,1);
    ...
    for(uint32_t rq=0;rq<n_requests;++rq){
        ...
        for(uint32_t L=0;L<n_layers;++L){
            ...
            sgemm_kernel<<<gg,gb,0,s_c>>>(dA,dB,dC,(int)gemm_n,(int)ci);
    ...
    if(verify){
        ...
        for(uint64_t b=0;b<n_hit;b+=std::max<uint64_t>(1,n_hit/16)){
            ...
            launch_fill_pattern(hk[b],0xFF,ts,s_r);
            CUDA_OK(cudaStreamSynchronize(s_r));
        ...
        for(uint64_t i=0;i<n_miss;i+=std::max<uint64_t>(1,n_miss/8)){
            ...
            launch_fill_pattern(mk[i],seed,ts,s_w);
            launch_fill_pattern(mv[i],seed^0xA5,ts,s_w);
            CUDA_OK(cudaStreamSynchronize(s_w));
            std::vector<uint64_t>one={b};
            std::vector<MemoryHandle>okm={mk_m[i]},ovm={mv_m[i]};
            do_write(L,one,okm,ovm);
            launch_fill_pattern(mk[i],0,ts,s_r);
            CUDA_OK(cudaStreamSynchronize(s_r));
            do_read(L,one,okm,ovm);
