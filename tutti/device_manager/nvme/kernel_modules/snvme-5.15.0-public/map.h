 ...
 *
 * NB: caller is responsible for rolling back ctrl->ioq_map_num /
 * ctrl->cq_num counters that NVM_MAP_* paths bumped while populating
 * these maps -- map.c has no visibility into per-ctrl accounting.
